...
## Addendum — v2 refinements (structural additions)

Net-new structural concepts to layer on top of the modules above:

- **Unified Learner Record / Student 360** — one governed learner-intelligence record (identity, curriculum, SEND/EAL/PP, baseline/current/target/predicted, verified study time, submission patterns, attendance, behaviour, emotional readiness, intervention history, reading/numeracy age, cognitive strengths, learning preferences, career interests, pathway, portfolio, skills passport, safeguarding visibility, parent-engagement level, consent). Not a contact database — the single record every agent reads and writes actions to.
- **Learner lifecycle + transition intelligence** — Applicant → Offered → Accepted → Pre-enrolment → Enrolled → Active → At-risk → Transferring → Leaver → Alumni; plus a **Transition Readiness Score** at each key-stage/phase jump (primary↔secondary, KS3↔GCSE, GCSE↔post-16, school↔university/apprenticeship/employment). A **portable Digital Learner Passport** (qualifications, verified skills, projects, work experience, endorsements) — a gap in Satchel and most MIS.
- **Staff directory + capability intelligence** — subjects/specialisms/year-group & SEND experience, languages, CPD, workload, intervention effectiveness; an **AI CPD Planner** (from misconceptions/marking/behaviour evidence), a **Workload Guardian** (marking/cover/after-hours burnout signals → redistribute or automate), and a **Staff Skill Marketplace** (who can mentor/cover/lead).
- **Governance: ABAC not just RBAC** — access decided by role × school × trust × department × class/student relationship × safeguarding status × data sensitivity × device/location/time × purpose × consent × delegation. Full role taxonomy (Trust Exec → LA Officer → Head → DSL → SENCO → Exams/Attendance/Behaviour/Admissions officers → Teacher → TA → Cover → Tutor → Counsellor → Parent/Carer → Student → Governor → Auditor). Non-negotiables: full audit trail, **reason-for-access logging**, restricted safeguarding notes, **break-glass** emergency access, auto access-expiry, download/watermark controls, suspicious-access alerts, 2FA, device-session mgmt, consent mgmt, retention automation, **SAR workflow**.
- **Dynamic AI cohorts** — continuously-updated groups (falling behind in X, predicted below target, high-attainers losing momentum, effort≫results, persistent non-submitters, attendance-affected, reading-intervention, burnout-risk, stretch-ready, NEET-risk) → **smart intervention groups** (members, objective, content, owner, cadence, success threshold, review date) → **impact tracking** (did attendance/attainment/confidence improve? completed? cost? continue/stop/change?).
- **Attendance event architecture** — every change is an immutable event (present/absent/parent-reports-illness/late/sign-in/moved-to-intervention/amended/authorised/evidence) retaining original+new value, source, user, timestamp, reason. A **conflict engine** (never silently overwrite two sources — create a conflict, rank trust, notify officer, preserve both). **AI Persistent-Absence Prevention** before the threshold. **Attendance→learning recovery**: absence auto-builds a catch-up pathway (missed lessons/assignments, essential objectives, recovery resources, schedule, review point).
- **Homework intelligence** — assignment states (Draft→Under review→Scheduled→Published→Started→Submitted→Late→Returned→Resubmission requested→Completed→Excused→Withdrawn→Archived); **Assignment Load Balancer** ("Year 10 already has 4 assignments + 2 tests Thursday — ~6.4h in 48h; suggest a better date"); **smart deadlines**; **academic-integrity controls** (AI-use declaration, draft history, oral-verification questions, teacher-set AI-assistance level); **missed-homework recovery** (why missed → shortened recovery task → reschedule → escalate only if needed → measure).
- **Notifications** — ranked (Emergency→Critical→Action→Deadline→Important→Info→Digest), quiet hours, escalation, fallback channel, **communication-fatigue control** (consolidate, de-dupe, best-performing channel), and **behavioural nudges** ("your maths task takes ~25 min; doing it at 6:30pm keeps your evening on track").
- **Integration Health Centre** — one-way/two-way selectable, source-of-truth designation, conflict detection, sync history, retry, field mapping, sandbox, health dashboard (last sync, failed records, duplicate users, unmatched classes, attendance write-back failures, permission failures + resolution).
- **Reclaim what Satchel removed** — governed **community + AI resource marketplace** (teacher/department/trust/public + AI, with quality scoring, curriculum verification, versioning, outcome data), **safe moderated academic discussion** (class/topic/homework-scoped, no private adult↔child DMs, safeguarding keyword detection, audit), a **school knowledge/policy drive** (versioned, approval, acknowledgement, AI summarise/translate/Q&A), full **AI lesson planning**, and **real class insights** (objective mastery, learning velocity, homework reliability, attendance/behaviour correlation, recommended groupings, intervention impact).
- **Audit + AI decision register** — audit every login/view/edit/export/AI-generation/AI-accept-reject/grade-change/attendance-change/safeguarding-access/permission-change; for every significant AI recommendation record prompt context, data used, model+version, recommendation, confidence, human decision, modification, final action, outcome.
- **Monetisation equity principle** — never paywall core child visibility (attendance, achievement, homework, behaviour, essential comms, academic risk). Charge for *added intelligence* (advanced Parent Adviser, university/career tools, portfolio, tutoring marketplace, extra ACUs) — the opposite of the Satchel One Plus reputational risk.
- **The four systems** StudYear should be: **System of record** (people, classes, curriculum, tasks, attendance, behaviour, assessments, comms) → **System of intelligence** (diagnostics, predictions, patterns, risks, recommendations, benchmarking) → **System of action** (plans, interventions, escalations, approvals, follow-ups) → **System of evidence** (outcomes, progress, engagement, intervention impact, audit, value delivered).

**Positioning:** *Satchel One organises school activity. StudYear turns school data into individual action and measurable progress* — detect every learning risk, build the right intervention, prove whether it worked.
