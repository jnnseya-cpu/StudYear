/* Shared school-teacher workspace chrome: base styles + sidebar on every teacher
   workspace page. Include AFTER guard.js:
     <script src="../nav.js" data-base="../" data-cur="classes/"></script> */
(function(){
  'use strict';
  ...
  var css=''+
  ':root{--navy-950:#060B18;--gold-300:#A9CFF2;--gold-400:#5FA8E0;--gold-500:#3D8FD1;--good:#5CBB7B;--warn:#c98500;--crit:#e66767;--ink:#EDF1F8;--ink-2:#AAB6CC;--ink-3:#6B7A96;--line:rgba(77,157,224,.14);--serif:Georgia,serif;--sans:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif}'+
  ...
  '.badge{font-size:10px;letter-spacing:.05em;border-radius:20px;padding:3px 9px;border:1px solid rgba(170,182,204,.25);color:var(--ink-2)}.badge.good{border-color:rgba(92,187,123,.45);color:var(--good)}.badge.warn{border-color:rgba(201,133,0,.45);color:#e0b566}.badge.crit{border-color:rgba(230,103,103,.45);color:#f0a0a0}.badge.info{border-color:rgba(77,157,224,.4);color:var(--gold-300)}'+
  ...
  '.live{font-size:11px;color:var(--good);border:1px solid rgba(92,187,123,.4);border-radius:20px;padding:3px 10px}'+
  ...
  /* shared data helpers for the whole workspace */
  window.TW=(function(){
    var SY=window.SY;if(!SY)return {};
    var profile=SY.get('teacherProfile',{});
    var schoolCode=profile.schoolCode||null;
    if(!schoolCode){try{for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);var m=k&&k.match(/^sy-school:([^:]+):roster$/);if(!m)continue;var r=SY.schoolGet(m[1],'roster',[]);if((r||[]).some(function(x){return (x.email||'').toLowerCase()===(SY.session.email||'').toLowerCase()})){schoolCode=m[1];break}}}catch(e){}}
    var schoolName=schoolCode?SY.schoolGet(schoolCode,'name','your school'):null;
    var verified=schoolCode?SY.schoolGet(schoolCode,'verified',false):false;
    function staffMe(){if(!schoolCode)return null;return (SY.schoolGet(schoolCode,'staff',[])||[]).find(function(s){return (s.email||'').toLowerCase()===(SY.session.email||'').toLowerCase()})}
    function myCohorts(){var me=staffMe();return (me&&me.cohorts)||[]}
    function roster(){return schoolCode?SY.schoolGet(schoolCode,'roster',[]):[]}
    function myStudents(){
      var own=profile.students||[];
      if(own.length)return own.map(function(s){return {email:s.email,name:s.name||SY.accountName(s.email)}});
      var coh=myCohorts();var r=roster();
      if(!coh.length)return r;/* empty assignment shows all school students */
      return r.filter(function(s){var pf=SY.readAccount(s.email,'profile',{});return coh.indexOf(pf.level)>=0||coh.indexOf(s.year)>=0});
    }
    function metrics(email){
      var qz=SY.readAccount(email,'quizzes',[]),diag=(SY.readAccount(email,'diagnostics',[])||[])[0]||null,plan=SY.readAccount(email,'plan',null),pf=SY.readAccount(email,'profile',{});
      var by={};qz.forEach(function(q){(by[q.subj]=by[q.subj]||[]).push(q.pct)});var subs=Object.keys(by);
      var overall=subs.length?Math.round(subs.reduce(function(a,s){return a+by[s].reduce(function(x,z){return x+z},0)/by[s].length},0)/subs.length):(diag?diag.avg:0);
      var total=plan&&plan.plan?plan.plan.reduce(function(a,w){return a+w.days.length},0):0,doneN=plan&&plan.done?Object.keys(plan.done).length:0;
      var hw=total?Math.round(100*doneN/total):0;
      var weak=subs.sort(function(a,b){return (by[a].reduce(function(x,z){return x+z},0)/by[a].length)-(by[b].reduce(function(x,z){return x+z},0)/by[b].length)})[0];
